  ...
  return (
    <nav className="navbar-container flex items-center justify-between px-8 py-4 border-b border-gray-200 bg-white">
      ...
      <div className="nav-links flex space-x-8 text-sm font-semibold text-gray-700">
        <Link
          to="/categorieen"
          className="nav-link flex items-center hover:text-black"
        >
          Categorieën <span className="dropdown-icon ml-1 text-xs">▼</span>
        </Link>
        <Link to="/producten" className="nav-link hover:text-black">
          Aanbiedingen
        </Link>
        <a href="#" className="nav-link hover:text-black">
          Blog
        </a>
      </div>
        ...
        {isLoggedIn ? (
          <div className="flex items-center space-x-4">
            ...
            {isAdmin ? (
              ...
              <Link
                to="/profiel"
                className="text-sm font-semibold hover:text-black"
              >
                Mijn Profiel
              </Link>
            )}
          ...
          <Link to="/login" className="account-button hover:text-black text-lg">
            👤
          </Link>
        )}
         {isLoggedIn ? (
