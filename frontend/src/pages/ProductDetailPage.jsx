...
export default function ProductDetailPage() {
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        const mockData = { 
          id: id || 1, 
          name: 'Bank "Rotterdam"', 
          description: 'Lorem ipsum l-orcalor sit amet, consectetur adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat.', 
          price: '899.00',
  ...
  if (isLoading) {
    return (
      <div className="page-container min-h-screen bg-gray-50 font-sans">
        <Navbar />
        <div className="loading-container text-center py-20 text-gray-500">Product laden...</div>
      </div>
    );
  }
  ...
  if (error || !product) {
    return (
      <div className="page-container min-h-screen bg-gray-50 font-sans">
        <Navbar />
        <div className="error-container text-center py-20 text-red-500">{error || 'Product niet gevonden.'}</div>
      </div>
    );
  }
      ...
      <main className="product-details-wrapper max-w-6xl mx-auto px-16 py-12 flex gap-12 bg-white mt-8 shadow-sm">
        ...
        <div className="product-images-section w-1/2">
          ...
          <div className="thumbnail-gallery flex space-x-4">
            {[1, 2, 3, 4].map((thumb) => (
              <div key={thumb} className="thumbnail-item w-20 h-20 bg-gray-100 flex items-center justify-center text-xs text-gray-400 cursor-pointer border hover:border-black transition">
                [Thumb {thumb}]
              </div>
            ))}
          </div>
        ...
        <div className="product-info-section w-1/2 flex flex-col justify-start pt-4">
          <h1 className="product-title text-3xl font-bold mb-2">{product.name}</h1>
          ...
          <div className="product-attributes text-sm mb-6 space-y-1 font-medium">
            <p>Afmetingen: <span className="font-normal text-gray-600">{product.dimensions}</span></p>
            <p>Materiaal: <span className="font-normal text-gray-600">{product.material}</span></p>
          </div>
